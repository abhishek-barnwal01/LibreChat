 ...
 * any of the selected filter values.
 * Uses case-insensitive substring contains: if the blob value contains
 * any selected value (or vice-versa), it's a match.
 * Compound values (e.g. "Godrej::Cinthol") are split into segments first.
 */
const matchesDatabricksFilter = (docValue: string | undefined, selectedValues: string[]): boolean => {
  ...
  return selectedValues.some((selected) => {
    ...
    return segments.some((seg) => {
      ...
      return segLower.includes(selLower) || selLower.includes(segLower);
    });
  });
};
